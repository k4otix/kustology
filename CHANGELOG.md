...
- **Function parameter names and `let`-bound call-site names are alpha-canonicalized.** A parameter is numbered `$param<i>` by declaration position and every reference inside that body follows, and a call site renames through the same map a reference does — so `let f = (w:int) {…}; f()` and `let g = (z:int) {…}; g()` are one digest, while a call to a name no visible `let` declared (a server-side function) is left as written. `declare pattern` parameters and `declare query_parameters` names stay verbatim: neither is a local label.
- **`LetBinding.inner_tables`/`inner_time_exprs` no longer reach the digest.** They are a derived index over the right-hand side beside them, so nothing is lost — but a copy of a name cannot be alpha-canonicalized, and while `inner_tables` was hashed a body reading a tabular parameter split on the written name after every `TableRef` beside it had been renamed. Both fields are populated on your IR exactly as before. **Every `let`-carrying digest moves**; the scheme tag stays `kustology-sem-v2`.
