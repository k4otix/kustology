...
- **Function parameter names and `let`-bound function call-site names are alpha-canonicalized.** A parameter is numbered `$param<i>` by declaration position and every reference inside that body follows, and a call site renames through the same map a reference does — so `let f = (w:int) {…}; f()` and `let g = (z:int) {…}; g()` are one digest. Only a `let` that bound a *function* renames its call sites: KQL resolves values and functions in separate namespaces, so `let abs = 5; … abs(x)` still calls the built-in. `declare pattern` parameters and `declare query_parameters` names stay verbatim: neither is a local label.
- **`LetBinding.inner_tables`/`inner_time_exprs` no longer reach the digest.** They are a derived index over the right-hand side beside them, so nothing is lost — but a copy of a name cannot be alpha-canonicalized, and while `inner_tables` was hashed a body reading a tabular parameter split on the written name after every `TableRef` beside it had been renamed. Both fields are populated on your IR exactly as before. **A digest moves exactly when one of its `let`s had put something in that index** — a scalar binding, or a pipeline naming no real table and no time function, indexed nothing and does not move; the scheme tag stays `kustology-sem-v2`.
...
- Operator/pipeline `result_schema` now comes from Microsoft's binder's `ResultType` instead of hand-written per-operator rules (tier 2), correcting join-collision suffixes, wildcard `project-keep`, `mv-expand` element types, `arg_max(t, *)`, and union conflicts. This correction moves nothing in `semantic_hash` by itself — `result_schema`/`table` are volatile, stripped from the payload before hashing — **the movement for any query containing an operator** (48 of 49 fixture queries) **comes from that stripping** (see `### Breaking`), still `kustology-sem-v2`.
...
- `ColumnRef.table` never carries the `$left`/`$right` sentinel — an unresolvable join side is `None` and `join_side` carries the side; `find` now seeds its tables for provenance like `search`.
...
- **A `let`-declared function's body, parameter types, defaults and `view` keyword now reach `semantic_hash` (tier 2).** The three documented let-function collisions are closed and `find_all(ir, TableRef)` sees inside a body, so the two tiers no longer disagree about a function-declaring query. **Every query declaring a `let` function moves digest**, as does one whose function body declares a `let` (those are now scoped to the body instead of hoisted into `QueryIR.let_bindings`); the scheme tag stays `kustology-sem-v2` under the one-tag-per-unreleased-window rule.
