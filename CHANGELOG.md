...
- **`scan` is modeled as a step machine** (tier 2). `ScanOp` drops `raw_text` for `with_match_id`, `with_step_name`, `declarations`, `order_by`, `partition_by` and `steps` (a new `ScanStep`), so `find_all(ir, ColumnRef)` reports the columns a step reads. Every `scan` query's `semantic_hash` moves.
=======
- **`scan` and `top-nested` are modeled with typed fields** (tier 2). `ScanOp` drops `raw_text` for `with_match_id`, `with_step_name`, `declarations`, `order_by`, `partition_by` and `steps` (a new `ScanStep`); `TopNestedOp` drops it for `levels` (a new `TopNestedLevel`, one per clause, carrying its count, key, aggregate, direction and `others` label). `find_all(ir, ColumnRef)` now reports the columns each reads, and every `scan` or `top-nested` query's `semantic_hash` moves.
>>>>>>> a388ba6 (feat(ir): model top-nested as a list of levels (#47))
=======
- **Operators that carried their own source text are modeled with typed fields** (tier 2). `scan`, `top-nested`, `graph-mark-components`, `graph-to-table` and `macro-expand` drop `raw_text` for typed clauses, so `find_all` reaches the columns and tables inside them. Every query using one of them gets a different `semantic_hash`, and a stored IR dump that carries `raw_text` for one fails validation.
>>>>>>> f434baa (feat(ir): model graph-mark-components, graph-to-table, and macro-expand (#47))
...
- **`semantic_hash` is computed on first read** (tier 2) and memoized, and `to_ir`'s `semantic_hash` parameter defaults to `False` (was `True`). The digest's cost, and any exception raised computing it, therefore moves from `to_ir()` to the first read of the field; `model_dump()` forces it. Pass `semantic_hash=True` to restore eager computation. No digest moves, and the field is never `""` — see [Storing hashes](docs/semantic-hash.md#storing-hashes) for what a copy and a reloaded dump report.
...
- `iter_elements()` and `plugin_functions()` (tier 1) — the latter reflects the `evaluate` plug-ins, previously absent from `all_function_names()`.
...
- **`let` bindings are enriched and their names resolve (tier 2).** The provenance pass now walks bindings in order and threads their names.
