...
- **Aggregate auto-names now come from Microsoft's own `ResultNameKind`/`ResultNamePrefix` symbol properties (tier 2)**, not a hand-written prefix table — irregulars (`buildschema(d)` → `schema_d`, `binary_all_and(a)` → `a`, and family) are named as the engine names them, in both bind states.
