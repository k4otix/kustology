...
| C8 | Tier 1 `_collect_table_refs` | `utils/analysis.py:125-167` omits `PartitionOperator`, `FindOperator` and `SearchOperator`. Verified: `find in (S1, S2) where X == 1` → `get_referenced_tables()` returns `set()`, and `replace_table("S1","New")` **silently returns the query unchanged** — the worst failure mode for a rewriting API, since a consumer migrating tables ships a query still pointing at the old one. | **fix** |
