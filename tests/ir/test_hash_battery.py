...
the same thing", and consumers deduplicate detection rules on it. A
canonicalization rule that is too aggressive or not aggressive enough breaks
that contract in one of two ways:

* A **collision**: two queries with different meaning hash the same, so a rule
  library drops one of them as a duplicate. This is the dangerous direction.
  Nothing fails and nothing crashes, and the missing rule surfaces only when
  the threat it would have caught gets through.
* A **split**: two queries with the same meaning hash differently, so a rule
  library keeps both as distinct entries. That is wasteful, and it is the
  failure this feature exists to prevent.

A minimal pair (two queries differing in exactly one respect) localizes either
failure to the one thing that changed. ``T | where a == 1`` against
``T | where a == 2`` failing points at the literal-value path; a large
realistic query failing says nothing about which of its twenty features is at
fault.

``MUST_DIFFER`` holds pairs that mean different things, one potential
collision each. ``MUST_EQUAL`` holds pairs that mean the same thing under a
different spelling, one potential split each. Both parse each query and
compare ``QueryIR.semantic_hash`` directly, without schema binding: no pair
here needs one, and binding roughly doubles the cost of a case for no
discriminating power. ``test_semantic_hash_bind_invariance.py`` covers the
bind-state axis.

``KNOWN_COLLISIONS`` holds pairs that mean different things and hash alike
today, open gaps in the model rather than regressions. They cannot go in
``MUST_DIFFER``, which they would keep red, and filing a real collision in
``MUST_EQUAL``, whose contract is "same meaning", would make the paragraph
above false. Its assertion is equality, so closing a gap turns the list red. A
consumer works around the collisions kustology discloses, so one vanishing
from the disclosure is a defect even when the behavior improved. **The list is
empty**, so nothing needs disclosing as a known gap. It stays for the next
one; see the comment above it.

Two datetime pairs need a caveat about what they prove.
``literal_value_and_ticks`` UTC-normalizes datetime literals so
``semantic_hash`` does not depend on the host timezone, and
``naive-vs-zulu-datetime`` guards that in-process on every host, not only
under this repository's usual dev and CI timezone. The obvious channel,
``ticks``, is host-offset-dependent: an unnormalized ``Local`` literal's raw
ticks diverge from an ``Unspecified`` literal's only where the host's UTC
offset is nonzero, and coincide at offset zero. ``LiteralExpr.value`` (the
``.ToString("o")`` rendering) is hashed too and diverges everywhere, a UTC
host included, because .NET's round-trip format writes an ``Unspecified``
value with no offset marker (``...0000000``) and a ``Local`` one with an
explicit marker (``...0000000+00:00``, zero offset included). Reading
``raw.Ticks`` and ``raw.ToString("o")`` with no ``Kind`` branch moves the
digest under both ``TZ=UTC`` and ``TZ=Asia/Tokyo``, so the pair discriminates
in either regime.

``tz-offset-vs-zulu-same-instant`` reads like a second guard for the same
rule. It is a same-instant sanity pin worth keeping, and it cannot fail on any
host. Probing the raw .NET node ahead of this library's normalization shows
``datetime(2024-01-01T05:00:00Z)`` and ``datetime(2024-01-01T00:00:00-05:00)``
both parsing through ``DateTime.Parse`` to ``Kind=Local`` with raw ``.Ticks``
of ``638396640000000000``. That follows from how .NET parses any
explicit-offset literal: the text resolves to its UTC instant using the offset
written in it, then re-projects onto the host's current offset. Two spellings
of one instant resolve to one UTC instant, and re-projecting one instant
through one host offset has one answer, so any two explicit-offset spellings
collapse to identical raw ticks on every host. Both sides of such a pair also
carry an offset marker in ``value``, so the asymmetry that makes
``naive-vs-zulu-datetime`` discriminating is unavailable to a pair of two
``Local`` values.

``tests/ir/test_literals.py::test_datetime_literal_is_utc_and_tz_independent``
covers the cross-process case this file cannot reproduce: a real
``TZ=Asia/Tokyo`` subprocess re-parse of the shipped code. ``datetime-value``
is worth keeping even though both its literals are bare (``Unspecified``), and
both ``MUST_EQUAL`` datetime pairs pin that the two ``Kind`` branches produce
a matching, self-consistent representation.

The WS4 block at the end of each list is the regression net for a family of
lossy-lowering bugs: the builder reaches one fully populated node from several
different KQL constructs, so nothing looks stubbed even though the digest has
lost the distinction between those constructs. Each such collapse is a
collision, which no other test in the suite is shaped to see. A per-task test
asserts that a field holds the right value, which stays true if a later change
stops that field from reaching the digest; these pairs assert the consequence
instead. Where a field carries KQL's effective default (D8) the pairing is a
``MUST_EQUAL``, since a bare ``join`` really does mean ``kind=innerunique``,
and the ``MUST_DIFFER`` runs against the spelling that names a different
operator.

``T | where "y" =~ X`` against ``T | where X =~ "y"`` is absent from
``MUST_EQUAL``. ``canonical()`` sorts commutative operand lists
(``and``/``or``/set membership) and sorts no ``BinOp``'s ``left``/``right``
for any operator, so a written operand swap on ``=~`` hashes differently from
the equivalent query with the literal on the other side. That limitation is
known and logged. ``lt-vs-gt-swapped`` below exercises it (``a < b`` against
``b > a``, one predicate hashed apart) because the task brief names it as a
required ``MUST_DIFFER`` case. It pins current behavior and claims nothing
about whether the behavior is right.
"""
...
MUST_DIFFER = [
    ...
    # `view`) and the body, statements and tail alike.
    #
    # A parameter's *name* is not among them. It is a local label like a `let`
    # name, alpha-canonicalized to `$param<i>` by declaration position (see
    # `let-function-parameter-name` in MUST_EQUAL). That has to be paid for
    # here: once the name stops mattering, *which* parameter a reference reads
    # has to start mattering, and a rename firing on a name it was never given
    # has to be caught. The next two rows are those two guards.
    (
        "let-function-param-positional",
        "let S = (a:int, b:int) { A | where x > a }; S(1, 2)",
        "let S = (a:int, b:int) { A | where x > b }; S(1, 2)",
    ),
    # The call-site rename is map-gated: a call is rewritten only when a
    # *visible* `let` declared that name. Neither `f` nor `g` is one, both
    # naming a server-side function the query never declares, so they must
    # still split, while the `n` beside them renames identically on both sides.
    (
        "let-function-body-server-call",
        "let n = 5; let S = () { A | where x > f(n) }; S()",
        "let n = 5; let S = () { A | where x > g(n) }; S()",
    ),
    # The other half of the gate: a visible `let` of the call's name is *not*
    # enough, because KQL keeps values and functions in separate namespaces.
    # `let abs = 5; T | extend y = abs(x) + abs` binds a value called `abs`,
    # still calls the built-in `abs`, and Microsoft's binder resolves both
    # readings with no diagnostic. A name-only gate would rename the call to
    # the binding's `$let<i>` and merge two queries computing different things,
    # so only a `let` that bound a *function* (`rhs_function`) renames its call
    # sites. The cluster resolves a stored server-side function of that name
    # the same way, which no schema in this suite can show. KS211 ("does not
    # refer to any known function") fires only where no function of the name
    # exists at all, so it cannot serve as the signal.
    (
    ...
    # -----------------------------------------------------------------------
    # WS4 -- one pair for each distinction the builder would lose by lowering
    # two constructs onto a single node, grouped by the task that models it.
    # -----------------------------------------------------------------------
    # 4.1 -- SortKey carries the ordering clause the parser wraps around the
    # expression. Direction and null placement decide the order rows come back
    # in, and three operators share that wrapper.
    ("sort-direction", "T | sort by a asc", "T | sort by a desc"),
    # `sort by a` defaults to `desc` (pinned below in MUST_EQUAL), so it must
    # still hash apart from the explicit `asc` spelling.
    ("sort-bare-vs-asc", "T | sort by a", "T | sort by a asc"),
    ("sort-nulls-placement", "T | sort by a nulls first", "T | sort by a nulls last"),
    # The nulls clause has a `None` default of its own, distinct from the
    # direction default above, so writing `nulls first` must reach the digest
    # relative to writing no nulls clause at all.
    ...
    # `desc` too (`top-bare-is-desc` in MUST_EQUAL).
    ...
    # the same wrapper, and its direction is optional, so unwritten is a third
    # distinct state alongside `asc` and `desc`.
    ...
    # 4.2 -- ForkBranch. `ForkOp.pipelines` carries each branch's body, so a
    # branch's contents, its name and its position are all in the digest.
    ...
    # 4.3 -- source position. A datatable carries its schema and rows,
    # externaldata has its own source class, and a qualifier or a wildcard on
    # a table name is read.
    ...
    # `columns` is the same name-vs-type point as the datatable pair above.
    # One case covers both halves here because the reader is shared
    # (`read_row_schema`) and pinned per half there.
    (
        "externaldata-format",
    ...
    # The corpus fixture `ExternalData_CsvFeed.kql` writes this modifier:
    # `ignoreFirstRecord` skips the CSV header, so the feed yields one fewer
    # row. A source node has no `raw_text`, so a with-clause entry the reader
    # skips survives nowhere.
    (
    ...
    # `with_itemindex=i` vs bare belongs to `test_operator_params.py`'s
    # `test_mv_expand_modifiers_all_reach_the_hash`, which pairwise-checks it
    # among five mutually-distinct variants, a stronger guarantee than one
    # pair here.
    ...
    # The value-level companion. The pair above compares a written modifier
    # against an absent one; here both sides write a type.
    (
    ...
    # them at its own call site, so each half of the duplication needs its own
    # pair.
    (
        "parse-where-flags",
    ...
    # Guards `join`/`lookup`'s effective default: a bare join records
    # "innerunique", and "inner" is a different operator. See the MUST_EQUAL
    # half for the effective default each records.
    ...
    # `FindOp.project` selects which columns come back.
    (
        "find-project",
    ...
    # TypedNameDecl: the declaration's Type child keeps a typed capture and an
    # untyped one apart.
    ...
    # 4.5 -- multi-statement queries. Every tabular statement reaches the
    # digest, so `T | count; U | count` does not hash as `T | count`.
    ...
    # 4.6 -- LetValueRef keeps a query-local constant and a real column of
    # that name apart.
    #
    ...
    # the binding is called `z` so `n` can only be a column of `T`. Holding the
    # binding count fixed is what makes the pair discriminating. Pairing
    # against a bare `T | where a > n` splits on the presence of the binding
    # alone and stays green under a use site lowered to `ColumnRef`.
    (
        "let-scalar-vs-column",
        "let n = 5; T | where a > n",
        "let z = 5; T | where a > n",
    ),
    # 4.7 -- typed nested pipelines. Typing `pipeline` keeps the subtree a
    # model across a reload, so the span-stripping walk reaches inside it and a
    # stored IR reproduces its own hash. These pairs pin that the nested query
    # is in the digest; the MUST_EQUAL half pins that its spans are not.
    ...
    # in `test_ir_builder.py::test_all_four_null_tests_hash_distinctly`, which
    # is pairwise-complete over all four and a strict superset of any pair
    # here.
    # Guards `datatable(...)` in expression position building a real
    # `DataTableExpr`. Falling through to `UnknownExpr` hashes raw source text,
    # which discriminates on value too and would pass this pair for the wrong
    # reason. See test_no_battery_pair_discriminates_on_an_unmodelled_blob.
    (
        "expr-datatable-values",
        'T | where a in ((datatable(x:string)["v"]))',
        'T | where a in ((datatable(x:string)["w"]))',
    ),
    # `declared_schema` and `declared_schema_star` carry the .NET
    # `EvaluateSchemaClause` (`EvaluateOperator.Schema`), so a written schema,
    # a different written schema, and no clause at all are three digests.
    (
    ...
    # replacing them, and `declared_schema_star` is what distinguishes it from
    # the non-star spelling with the same columns.
    (
        "evaluate-schema-star",
        "T | evaluate bag_unpack(d) : (*, x:string)",
        "T | evaluate bag_unpack(d) : (x:string)",
    ),
    # The five statement kinds, carried by `QueryIR.statements`. Nothing else
    # records that the statement was there, so without it two *different*
    # values of one statement hash alike.
    (
        "set-two-values",
    ...
    # and that one renames to `$let0` identically on both sides, leaving the
    # parameter name as the only thing that can split them. Adding
    # `TypedNameDecl` to `_LET_NAME_MODELS` would merge two queries that accept
    # different requests. A pair without the `let` could not see that, since
    # nothing in it would rename at all.
    (
    ...
    # `PatternMatch.body_lets` instead of reaching the digest through top-level
    # `let_bindings`. Same shape as `let-function-body-nested-let-value` above.
    (
...
MUST_EQUAL = [
    ...
    # The in-process, host-independent guard for datetime UTC-normalization.
    # Without it the two sides' LiteralExpr.value strings diverge in the
    # offset-marker channel on every host (Unspecified renders no marker,
    # Local renders one even at offset zero), and not only in ticks, which
    # diverge only on a non-UTC-offset host. See the module docstring.
    ("naive-vs-zulu-datetime", "T | where d > datetime(2024-01-01)", "T | where d > datetime(2024-01-01T00:00:00Z)"),
    # Same-instant sanity pin, and not a guard for host-timezone
    # independence: both spellings parse to Kind=Local with identical raw
    # .Ticks ahead of this library's normalization, on any host, so this pair
    # cannot fail. See the module docstring for the proof and for the real
    # guard (test_literals.py::test_datetime_literal_is_utc_and_tz_independent).
    (
    ...
    # did not write splits one query's two spellings into two digests. Each
    # pair below pins a decision about an unwritten or duplicate spelling.
    # -----------------------------------------------------------------------
    # D8 -- a required field carrying KQL's *effective* default, so the bare
    # spelling and the explicit one are one query and one digest. The value is
    # the one KQL applies: "innerunique" for a bare join, "leftouter" for a
    # bare lookup, neither of them the "inner" a naive read would assume.
    ("sort-bare-is-desc", "T | sort by a", "T | sort by a desc"),
    ("top-bare-is-desc", "T | top 5 by a", "T | top 5 by a desc"),
    # The same default through the *other* branch of the same expression:
    # `sort by a nulls first` has an Ordering node with no direction keyword,
    # where a bare `sort by a` has no Ordering node at all. One `or "desc"`
    # serves both, so the two pairs above exercise only the missing-node path
    # and would stay green if the keyword-absent path stopped defaulting.
    ("sort-nulls-only-is-desc", "T | sort by a nulls first", "T | sort by a desc nulls first"),
    # `order by` is a bare alias of `sort by`, the identical operator under a
    # different keyword, so the two spellings are one digest whether the
    # direction is written or bare.
    ...
    # field, so its default is a second decision that happens to match.
    (
    ...
    # `hints` is the one source-derived field excluded from the digest: a hint
    # asks the engine to execute the query differently and does not change the
    # rows it returns.
    ("join-hint-excluded", "T | join hint.strategy=shuffle U on a", "T | join U on a"),
    # Guards `FindOp.tables`, read with the no-argument `ToString()` overload:
    # under `IncludeTrivia.All` a comment written before a table name is part
    # of the name in the raw text, and reaches the digest through it.
    ("find-table-comment", "find in (// note\nT) where x == 1", "find in (T) where x == 1"),
    # A let-bound name is a local label: the hash renames every binding to its
    # declaration index. `LetValueRef` is what carries that rename to the use
    # site, which a `ColumnRef` there could not receive.
    ...
    # expressions, so formatting within the row does not reach the digest.
    (
        "expr-datatable-whitespace",
        'T | where a in ((datatable(x:string)["v"]))',
        'T | where a in ((datatable(x:string) [ "v" ]))',
    ),
    # `read_row_schema` reads the clause's columns as expressions, so
    # formatting within `evaluate`'s schema clause does not reach the digest.
    (
        "evaluate-schema-whitespace",
        "T | evaluate bag_unpack(d) : (x:string)",
        "T | evaluate bag_unpack(d) :  ( x : string )",
    ),
    # `named_param_name`/`named_param_value` strip trivia and serve every
    # named-parameter reader, so spacing inside `parse-kv`'s `with (...)`
    # clause does not reach the digest either.
    (
    ...
    # pipeline, so every canonicalization rule that holds for a top-level query
    # holds inside the braces too, with no special case for the body in
    # `transforms.py`. Formatting and comments first, then the signature.
    (
    ...
    # A parameter name is a local label too. Declaration position renames it
    # to `$param<i>` at the declaration and at every body reference the builder
    # lowered under the parameter's shadow, so two alpha-equivalent signatures
    # are one query.
    (
    ...
    # The same rename with the parameter read in the body, the half the row
    # above cannot see: renaming the declaration alone splits these two.
    (
    ...
    # a `ColumnRef`, and the row that needs the derived-index strip:
    # `LetBinding.inner_tables` records the body's table names as plain
    # *strings* no rename can reach, so `["T"]` and `["U"]` split these two
    # whenever that index reaches the digest.
    (
    ...
    # parameter (`$param<i>`, through the scope-local map), because the builder
    # commits that reference to the shadow reading and lowers it as a
    # `ColumnRef`. Two disjoint node classes, so their order does not matter.
    (
    ...
    # the body declares the duration `d`/`e` and `ago(d)` reads it, so the row
    # pins that the body's scope reaches a reference nested in a call argument.
    #
    # `LetBinding.inner_time_exprs` aliases the same `FuncCall` objects that
    # live inside `rhs_function`, so a rename walk would reach each one twice
    # if `transforms._DERIVED_INDEX_FIELDS` did not clear the index first.
    (
        "let-function-body-time-alias-rename",
        "let S = () { let d = 1h; A | where t > ago(d) | take 1 }; S()",
        "let S = () { let e = 1h; A | where t > ago(e) | take 1 }; S()",
    ),
    # The statement kinds. Whitespace proves a statement is *structure* in the
    # digest: a node that kept its own source would split every one of these.
    ...
    # inside a function body is. `_canonicalize_let_names` opens a child scope
    # for a `PatternMatch` the same way it does for a `LetFunction`; without
    # that branch these two split.
    (
    ...
    # reach `QueryIR.statements` or these two split on a local label.
    # `restrict-two-targets` above is the other half: the target still matters.
    (
...
# catch. They are filed here because a gap is an open boundary in the model
# rather than a regression, and a permanently red MUST_DIFFER entry would be
# deleted by the first person to run the suite. They are not in MUST_EQUAL
# either, whose contract is "same meaning, different spelling": filing a real
# collision there would make this module's own docstring false.
#
# **The list is empty**, so nothing is disclosed as a known gap. The list, the
# parametrization and the test stay, because the shape of the disclosure is
# what has value and the next gap needs somewhere to go. Pytest's "skipped:
# got empty parameter set" line is the standing signal that nothing is filed
# here.
#
# What qualifies a pair: the two queries mean different things, they hash
# alike *today*, and the reason is a construct the IR does not model. A
# canonicalization rule that over-merges is a bug instead, so it goes to
# MUST_DIFFER red and gets fixed. A pair filed here must also be disclosed
# everywhere the failure message below names, and mirrored as a row in
# `examples/semantic_hash_demo.py`'s KNOWN_MERGES, which the suite runs: that
# file prints the verdict for a reader and this one fails the build.
# ---------------------------------------------------------------------------
...
def test_no_battery_pair_discriminates_on_an_unmodelled_blob():
    ...
    A ``MUST_DIFFER`` pair proves nothing if the builder did not model either
    side. ``raw_text`` is in the digest payload for every node below the root,
    with only ``QueryIR``'s own copy excluded, so two queries differing in
    *any* text hash apart the moment one of them reaches a node that records
    its own source. A discriminator written against such a shape passes for a
    reason unrelated to the field it claims to guard, and would keep passing
    if that field were deleted. The risk is concrete: an operator that lowers
    to ``UnknownExpr`` carries ``raw_text``, so its direction pair would pass
